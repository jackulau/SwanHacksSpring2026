...
function NotesIndexPage() {
  ...
  const createPage = async () => {
    if (!user) return;
    try {
      // Relation fields (parent/course/lecture) reject empty strings on
      // PB 0.22+; omit them entirely instead of sending "".
      ...
      const e = err as { status?: number; message?: string; data?: { data?: Record<string, { message?: string }> } };
      const fields = e?.data?.data
        ...
        : "";
      alert(
        `Create failed (HTTP ${e?.status ?? "?"}): ${e?.message ?? "unknown"}\n\n${fields || "No field-level details."}\n\nFull error in Console (F12).`,
      );
    }
  };

  const archivePage = async (p: NotePage) => {
    if (!user) return;
    if (
      !window.confirm(
        `Move "${p.title || "Untitled"}" to Trash? You can restore it from there later.`,
      )
    ) {
      return;
    }
    try {
      await pb.collection("note_pages").update(p.id, { archived: true });
      setPages((prev) => prev?.filter((x) => x.id !== p.id) ?? null);
    } catch (err) {
      console.error("[notes] archive failed", err);
      alert("Couldn't move to Trash. Check the Console (F12) for details.");
    }
  };
...
function PageGrid({
  ...
  onArchive?: (p: NotePage) => void;
}) {
  return (
    <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {pages.map((p) => (
        <li key={p.id} className="group relative">
          <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 focus-within:opacity-100 inline-flex items-center gap-1">
            {onDuplicate && (
              <button
                type="button"
                onClick={(e) => {
                ...
                className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:border-[var(--color-primary)]"
              >
            ...
            {onArchive && (
              <button
                ...
                title="Move to Trash"
                className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-muted)] hover:text-[var(--color-error)] hover:border-[var(--color-error)]"
              >
